...
```text
x  "choose better cut points"        →  a dense schedule has no good cut points
o  "emit in an order that creates
...
```text
   1. from the current position, view [ +min_chunk , +region_cap ] as a window
   2. choose the cut candidate inside it with the fewest crossing values
      (ties → latest position, to keep pieces long)
   3. cut there, restart from step 1
```
