...
date: '2026-08-10'
tags: ['stablecoin', 'privacy', 'confidential-finance', 'payroll', 'zeto', 'zkp']
draft: false
category: 'Product'
...
![One pay run across the employer console, the GPU proving service, the pool contract, the indexer holding the auditor key, and a recipient wallet](/blog/confidential-stablecoin-infrastructure/pay-run-sequence.svg)
...
Everything in this post is public:
...
- [Payroll console](https://payroll.fractalyze.io): the employer side. Builds a 255-recipient worksheet and settles it as one private disbursement. Testnet, access-gated; the proving service behind it is institution-side and runs on demand rather than as a public endpoint.
...
- [BongtuPool on Base Sepolia](https://sepolia.basescan.org/address/0x2a72fea8e97fF79069B3D0165A5DB1Fef7F9322C): the live pool contract. The explorer shows bytecode only, so read the source in the repo at the deploying commit rather than taking the explorer's word for it. The gas in this post is measured against the deployed contracts, transaction by transaction, and [`docs/performance.md`](https://github.com/fractalyze/bongtu/blob/main/docs/performance.md) carries the receipts.
...
Measured, not estimated: one 256-recipient payout is 3,905,519 L2 gas, which is 15,256 gas per recipient, roughly 90 times cheaper per recipient than Zeto's own published figures. Proving one batch takes about 1.6 seconds on a single GPU, of which the proof itself is 0.47 seconds for a circuit of 2.8 million constraints. A 100,000-person payroll is 391 such batches: about ten minutes of proving, and at the chain's current gas price roughly 0.009 ETH in fees, well under \$50.
